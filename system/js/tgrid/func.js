function Crea_Objeto(elemento, tipo,objeto) {
	...
	switch(tipo) {
		...
		case 'combo':
			...
			var contador = 0;
			$.each(objeto[id_separados[1]], function(id, contenido) {
...
function muestra_detalle_post(funcion, fila, parametro, id_tabla, tipo){
	...
	if(cont != undefined){
		...
		arreglo_valores += " \"909\" : \"123\"}";
		
		if(tipo == 'html'){
			$.ajax({
				url : sUrlP + funcion,
				type : "POST",
				data : "objeto=" + arreglo_valores,
...
function EjecutarEnlace(funcion, fila, parametro, id_tabla,metodo,enlace) {
	...
	if (parametro == null) {//ENVIAR TODAS LAS COLUMNAS A LA FUNCION A EJECUTAR
		$("#" + fila).each(function(index) {
			...
			$(this).children("td").each(function(index2) {
				if (this.id.charAt(0) == 'R') {//PARA OBTENER EL VALOR EN CASO DE OPTION BUTTON
					...
					if (this.childNodes.length > 1) {
						var algo = this.childNodes.item(1);
						arreglo_valores[i] = algo.innerHTML ;
						i++;
					} else {
						arreglo_valores[i] = $(this).text() ;
		...
		for (var j = 0; j < valores.length; j++) {//VERIFICAR QUE COLUMNAS SE VAN A ENVIAR A LA FUNCION A EJECUTAR
			...
			if (id.charAt(0) == 'R') {
				...
				var hijos = document.getElementById(id).childNodes.length;
				if (hijos > 1) {
					var texto = document.getElementById(id).childNodes.item(1).innerHTML;
					arreglo_valores[j] = texto ;
				} else {
					arreglo_valores[j] = $("#" + id).text();
				}
	...
	if(metodo == 1){
		...
		for(k=0;k<arreglo_valores.length;k++){
			if(k==0){
				parametro_ejecuta += '(';
			}else{
				parametro_ejecuta += ' , ';
			}
			parametro_ejecuta += '\'' + arreglo_valores[k] + '\'';
		}
		parametro_ejecuta += ');';
